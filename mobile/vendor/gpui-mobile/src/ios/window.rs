...
impl IosWindow {
    pub fn new(handle: AnyWindowHandle, _params: WindowParams) -> anyhow::Result<Self> {
        ...
        unsafe {
            ...
            let metal_instance = wgpu::Instance::new(wgpu::InstanceDescriptor {
                ...
                // wgpu v29 requires a display handle even for `create_surface_unsafe`:
                // with `display: None` and no explicit raw display handle it errors
                // with `MissingDisplayHandle` before the Metal surface is built.
    ...
    pub(crate) fn pump_momentum(&self) {
        // `try_borrow_mut`: the ScrollWheel events below can synchronously
        // re-enter this handler via GPUI's refresh → request_frame path;
        // a plain borrow would panic on the re-entrant frame.
        let mut scroller = match self.momentum_scroller.try_borrow_mut() {
            Ok(scroller) => scroller,
            Err(_) => return,
        };
    ...
    pub fn notify_active_status_change(&self, is_active: bool) {
        ...
        // `try_borrow_mut`: the launch sequence fires willEnterForeground then
        // didBecomeActive back-to-back; a re-entrant borrow panics.
        if let Ok(mut borrowed) = self.active_status_callback.try_borrow_mut() {
            if let Some(callback) = borrowed.as_mut() {
                // `catch_unwind`: once `Application::run` returns (iOS
                // `Platform::run` does not block), the `AppCell` may already
                // be released, and the GPUI callback panics on the dangling
                // `Weak`. Catch it so a late lifecycle notification cannot
                // crash the process — the app simply keeps the last state.
    ...
    pub fn handle_layout_change(&self) {
        unsafe {
            ...
            // `try_borrow_mut`: a synchronous re-layout from inside the
            // callback re-enters this handler during launch.
            let cb = self
                .resize_callback
                .try_borrow_mut()
                .ok()
...
impl Drop for IosWindow {
    fn drop(&mut self) {
        // Remove this window from the FFI window list so later lifecycle
        // callbacks (didBecomeActive, request_frame) never dereference a
        // dangling pointer. The list is main-thread only, matching how
        // register_with_ffi appends.
