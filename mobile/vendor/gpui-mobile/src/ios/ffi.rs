...
/// Remove a window from the FFI window list. Called from `IosWindow::drop`
/// so lifecycle callbacks never dereference a freed window.
pub(crate) fn unregister_window(window: *const super::window::IosWindow) {
    if let Some(wrapper) = IOS_WINDOW_LIST.get() {
        unsafe {
            (*wrapper.0.get()).retain(|&candidate| candidate != window);
        }
    }
}
...
pub extern "C" fn gpui_ios_request_frame(window_ptr: *mut c_void) {
    ...
    // as the callback might try to borrow the same RefCell. Use
    // `try_borrow_mut` so a synchronous re-entry (a render callback
    // that triggers another request_frame) skips this frame instead of
    // panicking.
    let Ok(mut slot) = window.request_frame_callback.try_borrow_mut() else {
        return;
    };
    let callback = slot.take();
    if let Some(mut cb) = callback {
        drop(slot);
        // `catch_unwind`: same rationale as `notify_active_status_change` —
        // after `Application::run` returns the AppCell may be gone, so a
        // frame callback would panic on the dangling Weak. Skip the frame
        // instead of crashing.
